...
신청·접수·예약·등록·조회·설치 등 실제 처리가 필요하면 규정에 적힌 담당 부서로 안내한다.
시스템명·메뉴 클릭 경로를 안내하거나 챗봇이 신청·조회·처리를 대신할 수 있다고 말하지 않는다.
규정의 금액·기한·증빙·승인 조건은 함께 안내하되, 담당 부서가 없으면 임의로 만들지 않는다.
답변 마지막 줄에는 사용한 근거를 [규정명 > 항목명]으로 표시한다. 아래 완성된 대괄호 표기 중 실제 근거로 사용한 것을 그대로 복사한다:
{source_choices}
규정명과 항목명을 모두 표시하며 서로 다른 문서의 이름과 항목을 섞지 않는다. 근거가 여러 규정에 있으면 각각 표시한다.
인용 표기 앞에 '출처:'나 별도 제목을 붙이지 않는다. 파일 경로와 .md 확장자는 표시하지 않는다.
...
답변 전에 금액·시각·기한·담당 부서와 출처가 본문에 맞는지 확인하고, 점검 과정 없이 최종 답변만 출력한다.

[지원 범위]
개인별 잔여 연차·실수령액 등 실제 정보 조회는 지원하지 않는다. 확인이 필요하면 문서의 담당 부서로 안내한다.
...
def build_system_prompt(conversation_title: str, question: str, history: list[dict] | None = None) -> str:
    ...
    # 이미 읽은 본문에서 파일명과 항목명을 연결해 실제 MD를 찾을 수 있게 한다.
    source_names = []
    document_name = ""
    for line in regulations.splitlines():
        if line.startswith("문서: "):
            document_name = Path(line.removeprefix("문서: ")).stem
        elif line.startswith("## ") and document_name:
            source_names.append(f"[{document_name} > {line.removeprefix('## ')}]")
    source_choices = " ".join(dict.fromkeys(source_names)) or "(인용할 항목 없음)"
